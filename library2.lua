--[[
	...
	Updated with fullscreen support, collapse button, and logo/image support
	
	USAGE:
		local SlickUI = loadstring(game:HttpGet("..."))()
		local Window = SlickUI:CreateWindow('Title', {
			Logo = 'https://raw.githubusercontent.com/emekaobierika687-png/SL-HUB/refs/heads/main/libraray1.png',
...
function SlickUI:CreateWindow(title, options)
	...
	-- Minimize/Collapse Button (replaces X, now collapses to small size)
	...
	local savedPosition = windowPosition
	local savedSidebarWidth = sidebarWidth

	-- Collapse function
	local function toggleCollapse()
		isCollapsed = not isCollapsed
		if isCollapsed then
			-- Collapse to small bar (like in the image)
	...
	local function toggleFullscreen()
		...
		if isFullscreenState then
			...
			for _, child in ipairs(Main:GetChildren()) do
				if child:IsA("Frame") and child.Size == UDim2.new(0, savedSidebarWidth, 1, -53) then
					child.Size = UDim2.new(0, 180, 1, -53)
				end
			end
		else
			-- Go windowed
			...
			for _, child in ipairs(Main:GetChildren()) do
				if child:IsA("Frame") and child.Size == UDim2.new(0, 180, 1, -53) then
					child.Size = UDim2.new(0, savedSidebarWidth, 1, -53)
				end
			end
		end
	end
